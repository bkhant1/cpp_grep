...
// 38 "/usr/include/c++/5/exception" 2 3
...
// 35 "/usr/include/c++/5/typeinfo" 2 3
...
struct has_begin
{
	template <class Tp> static char 
	test( decltype(std::declval<Tp>().begin(), void())) {}; 

	template <class Tp> static int 
	test(...) {}; 

	static const bool result = sizeof(test<T>(0)) == sizeof(char); 
};

template <class T> struct has_end { template <class Tp> static char test(decltype(&Tp::end)) {}; template <class Tp> static int test(...) {}; static int type_of(int){}; static const bool result = sizeof(test<T>(0)) == sizeof(char); };
template <class T> struct is_iterator { template <class Tp> static char test(typename std::iterator_traits<Tp>::iterator_category*) {}; template <class Tp> static int test(...) {}; static int type_of(int){}; static const bool result = sizeof(test<T>(0)) == sizeof(char); };
...
struct is_iterable
{
 ...
 using begin_return_or_void =
  typename hh_if_then_else<
   has_begin_and_end,
   get_return_type,
   void_
  >::template apply<decltype(&T::begin)>::type;

 static constexpr bool begin_return_is_iterator =
  is_iterator<begin_return_or_void>::result;

 static constexpr bool value =
  has_begin_and_end &&
  begin_return_is_iterator;
};


template<class T, class S, class Parg>
std::list<T&> grep(S s, bool(*)(Parg))
{

 constexpr bool s_is_iterable = is_iterable<S>::result;
...
int main(int argc, char* argv[])
{
 ...
 std::cout << has_begin<HasBegin>::result << std::endl;
 std::cout << has_begin<HasParentBegin>::result << std::endl;
 std::cout << has_begin<NotAsBegin>::result << std::endl;
 std::cout << is_iterator<NotAsBegin>::result << std::endl;
 std::cout << is_iterator<IVec::iterator>::result << std::endl;

 print_type<
  if_then_else<true, X<int>, X<char>>::type::type>();
 print_type<
  if_then_else<false, X<int>, X<char>>::type::type>();

 print_type<
  get_return_type<decltype(&my_function)>::type>();
 print_type<
  get_return_type<decltype(&my_function_2)>::type>();
 ...
 print_type<std::iterator_traits<std::vector<int>::iterator>::iterator_category>();
}
