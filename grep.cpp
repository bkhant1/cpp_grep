...
struct splice_grep_result_in
{
	...
	bool operator() (S& s)
	{
		m_l.splice(m_l.end(), grep<T>(s, m_p));
	}
	...
	grep_helper_base<T,S,P>
{
	...
	std::list<T*> operator() ()
	{
		...
		for (auto& x: base::s())
			ret.splice(ret.end(), grep<T>(x, base::f()));
...
{	
	...
	sizeof(std::declval<HasBegin>());
	std::cout << "Has begin tests" << std::endl;
	std::cout << has_begin<HasBegin>::result << std::endl; // 1
	std::cout << has_begin<HasParentBegin>::result << std::endl; // 1
	std::cout << has_begin<NotAsBegin>::result << std::endl; // 0
	...
	std::list<char*> result_complex_2 = grep<char>(a_complex_one, equal_to_char_B);
	print_grep_result(result_complex_2); // { B, B} (as string is iterable)
	std::cout << "Nb eval: " << equal_to_char_B.m_nb_eval << std::endl; // 8
}
